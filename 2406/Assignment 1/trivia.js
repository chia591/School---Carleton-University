...
function trivia(){
	...
	document.getElementById("btnRandom").addEventListener("click", btnRandom);
}
...
function btnLoad(){
	...
	switch(document.getElementById("dropDown").value){
		case "1":
			console.log(tests["Test 1"]);
			buildQuestion(tests["Test 1"]);
			selectedTest = tests["Test 1"];
			console.log(testingArea);
			break;

		case "2":
			console.log(tests["Test 2"]);
			buildQuestion(tests["Test 2"]);
			selectedTest = tests["Test 2"];
			break;

		case "3":
			console.log(tests["Test 3"]);
			buildQuestion(tests["Test 3"]);
			selectedTest = tests["Test 3"];
			break;

		case "4":
			console.log(tests["Test 4"]);
			buildQuestion(tests["Test 4"]);
			selectedTest = tests["Test 4"];
			break;

		case "5":
			console.log(tests["Test 5"]);
			buildQuestion(tests["Test 5"]);
			selectedTest = tests["Test 5"];
			break;

		case "6":
			console.log(tests["Test 6"]);
			buildQuestion(tests["Test 6"]);
			selectedTest = tests["Test 6"];
			break;
	...
	//Added buttons at the end
	let btnClear = document.createElement("button");
	btnClear.id = "btnClear";
	btnClear.textContent = "Clear Test";
	btnClear.onclick = btnClearf;
	
	let btnCheck = document.createElement("button");
	btnCheck.id = "btnCheck";
	btnCheck.textContent = "Check test";
	btnCheck.onclick = btnCheckf;

	testingArea.appendChild(btnClear);
	testingArea.appendChild(btnCheck);
}

function btnClearf(){ 
	let confirmation = confirm("Are you sure you want to clear the test?")
	let childrens = document.getElementsByClassName("Questions");
	if(confirmation){
		for(let x = 0; x < childrens.length; x++){
			let child = childrens[x].childNodes;
			child[0].style.backgroundColor = "rgba(255, 255, 255, .4)"
			for(let y = 1; y < child.length; y++){
				child[y].style.backgroundColor = "";
				child[y].childNodes[0].checked = false;
				child[y].childNodes[2].data = " ";
...
//Checks that every question has one anwser selected then grades the test
function btnCheckf(){
	let hasAnwser;
	let childrens = document.getElementsByClassName("Questions");
	console.log(childrens);
	//Checks if all the questions has been anwsered
	for(let x = 0; x < childrens.length; x++){
		let child = childrens[x].childNodes;
		hasAnwser = false;
		for(let y = 1; y < child.length; y++){
			if(child[y].childNodes[0].checked == true){
				hasAnwser = true;
				break;
			}
		}
		if(hasAnwser == false){
			child[0].style.backgroundColor = "rgb(255, 191, 202)";
			completed = false;
		}else{
			child[0].style.backgroundColor = "rgba(255, 255, 255, .4)";
		}
	}
	if(hasAnwser == false){alert("Hey! You didnt complete the test. Go over it again and anwser all the questions");}

	//If the entire test is completed, the test is then evaluated
		...
		for(let x = 0; x < childrens.length; x++){
			...
			let child = childrens[x].childNodes;
			let y;
			for(y = 1; y < child.length; y++){
				if(child[y].childNodes[0].checked == true){
					if(child[y].childNodes[1].data == selectedTest[x].correct_answer){
						child[y].style.backgroundColor = "rgb(130, 224, 170)";
						child[y].childNodes[2].data = "✔️";
						console.log(child[y].childNodes);
					}else{
						child[y].style.backgroundColor = "rgb(255, 191, 202)";
						child[y].childNodes[2].data = "❌";
						console.log(child[y].childNodes);
					}
					break;
...
//constructructs every question
function buildQuestion(test){
	console.log("going");
	for (let x = 0; x < test.length; x++){
		let question = document.createElement("div")
		//Print question
		let title = document.createElement("label");
		title.appendChild(document.createTextNode(test[x].question));
		title.appendChild(document.createElement("br"));
		title.style.backgroundColor = "rgba(255, 255, 255, .4)";
		question.appendChild(title);
		
		//question set up
		let anwsers = new Array(test[x].incorrect_answers.length + 1);
		for(let y = 0; y < anwsers.length - 1; y++){
			anwsers[y] = makeRadioButton(x, "N", test[x].incorrect_answers[y]);
		}
		anwsers[anwsers.length - 1] = makeRadioButton(x, "Y", test[x].correct_answer);
		shuffle(anwsers);

		//print questions
		for(let y = 0; y < anwsers.length; y++){
			question.appendChild(anwsers[y]);
		}
		question.className = "Questions"
		testingArea.appendChild(question);
		testingArea.appendChild(document.createElement("br"));
		testingArea.appendChild(document.createElement("br"));
	}
}
...
function btnRandom(){
	...
	x.onreadystatechange = function(){
		if(x.readyState == 4 && this.status == 200){
			selectedTest = JSON.parse(this.responseText);
			if(selectedTest["response_code"] == 0){
				while(testingArea.firstChild){
					testingArea.removeChild(testingArea.firstChild);
				}

				selectedTest = selectedTest["results"]
				buildQuestion(selectedTest); // MAIN TEST BUILD FROM REQUEST
	...
	x.open("GET", "https://opentdb.com/api.php?amount=10", true);
	x.send();
}
