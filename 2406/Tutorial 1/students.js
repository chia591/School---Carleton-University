...
students.map(function (students) { console.log(students.fname); });

let x = students.filter(function (students) { return students.egrade >= 80});
x.map(function (x) { console.log(x.fname + " " + x.lname)});

let y = 0.0; let z = 0;
students.map(function (students) { y += (students.agrade * 0.4) + (students.tgrade * 0.1) + (students.egrade * 0.5); z++});
console.log(y / z);